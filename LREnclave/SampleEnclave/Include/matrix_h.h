...
class matrix{
  ...
  float* data;      // where the data contents of the matrix are stored
  void allocate()   {
    delete[] data;
...
public:
 ...
 /*   void evaluateSigmoidFunc(matrix& input)  {
        actualsize = input.getactualsize();
        if ( maxsize < input.getactualsize() )   {
            maxsize = input.getactualsize();
            allocate();
        }
        for ( int i = 0; i < actualsize; i++ )
            for ( int j = 0; j < actualsize; j++ )  {
